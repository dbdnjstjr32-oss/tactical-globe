...
// ==========================================
// 💾 [INDEXEDDB MAP TILE CACHE INFRASTRUCTURE]
// ==========================================
class TileDatabase {
  ...
  constructor() {
    ...
    this.dbPromise = new Promise((resolve, reject) => {
      const request = indexedDB.open("maplibre-tile-cache", 1);
  ...
  async getTile(url: string): Promise<ArrayBuffer | null> {
  ...
  async saveTile(url: string, buffer: ArrayBuffer): Promise<void> {
...
const tileDb = typeof window !== "undefined" ? new TileDatabase() : null;

// Register custom cache protocol for MapLibre
if (typeof window !== "undefined") {
  try {
    MapLibreGL.addProtocol("cached-tiles", (params, abortController) => {
      const url = params.url.replace("cached-tiles://", "");
      ...
      return new Promise((resolve, reject) => {
        ...
        if (tileDb) {
          tileDb.getTile(url).then((cachedData) => {
            if (cachedData) {
    ...
    console.warn("cached-tiles protocol already registered or failed:", e);
  }
}
...
function MarkerPopup({
  ...
  useEffect(() => {
    ...
    // Automatically open popup on next tick when marker is mounted to the map
    const timer = setTimeout(() => {
      try {
        ...
        console.warn("Failed to auto-open popup:", e);
      }
    }, 0);
